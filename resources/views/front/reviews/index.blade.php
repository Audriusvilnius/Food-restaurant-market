...
@section('content')
    ...
    <style>
        ...
        .average-rating::before {
            ...
            background: linear-gradient(90deg, gold {{ ($food->rating / 5) * 100 }}%, rgba(0, 0, 0, 0.2) {{ ($food->rating / 5) * 100 }}%);
    ...
    <div class="container mb-5" style="min-height: 850px">
            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <form action="{{ route('update-rate') }}" method="post">
                        ...
                        <div class="col-md-12 d-flex">
                            <div class="card-body">
                                <textarea class="form-control" placeholder="{{ __('Leave Review here') }}" name="food_review" rows="5"
                                    cols="50"></textarea>
                            </div>
                        </div>
                        <div class="col-md-12 d-flex">
                            <div class="card-body">
                                <a href="{{ route('start') }}"
                                    class="btn btn-secondary float-start">{{ __('HOME') }}</a>
                                <button type="submit"
                                    class="btn btn-outline-secondary float-end">{{ __('RATE & REVIEW') }}</button>
                            </div>
                        </div>
                </div>
                @csrf
        ...
        <section class="py-1 text-center container">
            <div class="col-lg-4 col-md-8 mx-auto mt-1 py-2">
                @if (Session::has('ok'))
                    <h6 class=" alert alert-success alert-dismissible fade show border border-dark border-2"
                        role="alert">{{ Session::get('ok') }}
